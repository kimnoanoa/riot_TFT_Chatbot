...
def api_chat():
    ...
    # ✅ 오피덱 / 사기덱 / 메타덱 질문 대응
    if any(k in user_msg for k in ["오피", "사기덱", "op", "요즘 오피", "메타덱","사기 덱","오피 덱","op덱","op 덱" ,"개사기덱"," 메타 덱","좋은 덱","좋은덱"]):
        op_decks = [
            {
                    "name": "수정 갬빗 말자하 덱",
            ...
            },
            {
                    ...
                    "core": "세나, 자르반, 아칼리, 라이즈, 아트록스, 다리우스 , 요네",
    ...
    # ✅ 일상 대화 응답
    if any(k in user_msg for k in ["점심", "밥", "먹을까", "저녁", "식사", "배고파"]):
        lunch_menu = random.choice(["김치찌개", "초밥", "제육볶음", "떡볶이", "샐러드", "라멘", "비빔밥"])
